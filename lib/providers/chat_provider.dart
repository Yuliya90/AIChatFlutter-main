...
class ChatProvider with ChangeNotifier {
  ...
  Future<void> _initializeProvider() async {
    try {
      ...
      // Если данные аутентификации найдены, инициализируем API клиент
      if (authData != null) {
        await _api.initialize(authData.apiKey);
        _log('API client initialized with saved key');
      } else {
        _log('No auth data found, API client not initialized');
      }
